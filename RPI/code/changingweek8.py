...
from settings import API_IP, API_PORT, API_PORT_ALGO
...
class RaspberryPi:
    ...
    def __init__(self):
        ...
        self.current_location = self.manager.list()
    ...
    def command_follower(self) -> None:
        ...
        while True:
            ...
            # Snap command
               ...
               # print("IT IS RUNNING BEFORE cur_location")
                cur_location = self.path_queue.get_nowait()
                print("THIS IS THE CURLOCATION")
                print(cur_location)
               # self.current_location[0] = cur_location[0]
               # self.current_location[1] = cur_location[1]
               # print("THIS IS Y AXIS")
               # print(cur_location[1])
               # self.current_location[2] = cur_location[2]
               ...
               # self.logger.info(
                ...
                print("WJDIOSHJDSJIHODHOISDHIOPDSHOIPSDHOIPSDHOIP")
    ...
    def rpi_action(self):
        ...
        while True:
            ...
            if action.cat == "obstacles":
                ...
                self.snap_and_rec(action.value)
    ...
    def snap_and_rec(self, obstacle_id: str) -> None:
        ...
        """
        # obstacle_id = obstacle_id_with_signal.split("_")
        self.logger.info(f"Capturing image for obstacle id: {obstacle_id}")
        self.android_queue.put(AndroidMessage(
            "info", f"Capturing image for obstacle id: {obstacle_id}"))
        url = f"http://{API_IP}:{API_PORT}/notify-upload/{obstacle_id}"
        filename = f"snap_{obstacle_id}_wk8.jpg"
        ...
        self.logger.info(
            f"Image recognition results: {results} ({SYMBOL_MAP.get(str(results['id']))})")
       ...
       # if results['id'] == 0:
        ...
        # Notify android of img
        self.android_queue.put(AndroidMessage("imgreg", results))


    def request_algo(self, data, x=1, y=1, d=0, retrying=False):
        """
        ...
        # import ...SC2079_MDP_ALGO-main.continuous_space.endpoint
        bodydict = {'obstacles': data}
        self.logger.info(f"data: {data}")
        body = {**bodydict}
        url = f"http://{API_IP}:{API_PORT_ALGO}/path"
        ...
        for p in path:  # ignore first element as it is the starting position of the robot
